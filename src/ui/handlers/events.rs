...
use crate::{
    ...
    ui::{
        ...
        templates::{EventListPartial, EventsTemplate, GlobalPauseInfo},
    },
};
use sqlx::Sqlite;
...
pub async fn events_default(
    ...
    if is_htmx_request(&headers) {
        ...
        // Fetch global pause status for full page
...
#[derive(Deserialize)]
pub struct EventsByIpQuery {
    pub next_token: Option<String>,
}

pub async fn events_by_ip(
    State(state): State<ConcreteState>,
    Path(ip_str): Path<String>,
    headers: HeaderMap,
    Query(query): Query<EventsByIpQuery>,
) -> Result<Response, ApiError> {
    // Parse IP address
    let client_ip = ip_str
        .parse::<IpAddr>()
        .map_err(|_| ApiError::BadRequest("Invalid IP address".into()))?;
